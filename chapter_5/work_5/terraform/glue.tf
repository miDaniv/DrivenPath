resource "aws_glue_catalog_database" "driven_data_db_mykhailo" {
  name = "driven_data_db_mykhailo"
...
resource "aws_glue_crawler" "raw_driven_data_crawler" {
  ...
  role          = aws_iam_role.glue_execution_role_mykhailo.arn
  database_name = aws_glue_catalog_database.driven_data_db_mykhailo.name
  description   = "Crawls data in S3 and creates table in Glue database for raw data."
  s3_target {
    path = "${aws_s3_bucket.driven_data_bucket_mykhailo.bucket}/data/raw/"
  }
  tags = {
    Name = var.tag
  }
...
resource "aws_glue_crawler" "staging_dim_address_crawler" {
  ...
  s3_target {
    path = "${aws_s3_bucket.driven_data_bucket_mykhailo.bucket}/data/staging/dim_address/"
  }
  tags = {
    Name = var.tag
  }
...
resource "aws_glue_crawler" "staging_dim_date_crawler" {
  ...
  s3_target {
    path = "${aws_s3_bucket.driven_data_bucket_mykhailo.bucket}/data/staging/dim_date/"
  }
  tags = {
    Name = var.tag
  }
...
resource "aws_glue_crawler" "staging_dim_finance_crawler" {
  ...
  s3_target {
    path = "${aws_s3_bucket.driven_data_bucket_mykhailo.bucket}/data/staging/dim_finance/"
  }
  tags = {
    Name = var.tag
  }
...
resource "aws_glue_crawler" "staging_dim_person_crawler" {
  ...
  s3_target {
    path = "${aws_s3_bucket.driven_data_bucket_mykhailo.bucket}/data/staging/dim_person/"
  }
  tags = {
    Name = var.tag
  }
...
resource "aws_glue_crawler" "staging_fact_network_usage_crawler" {
  ...
  s3_target {
    path = "${aws_s3_bucket.driven_data_bucket_mykhailo.bucket}/data/staging/fact_network_usage/"
  }
  tags = {
    Name = var.tag
  }
...
resource "aws_glue_job" "staging_dim_address_glue" {
  ...
  role_arn    = aws_iam_role.glue_execution_role_mykhailo.arn
  command {
    name            = "pythonshell"
    python_version  = "3.9"
    script_location = "s3://${aws_s3_bucket.driven_data_bucket_mykhailo.bucket}/tasks/dim_address.py"
  }
  tags = {
    Name = var.tag
  }
...
resource "aws_glue_job" "staging_dim_date_glue" {
  ...
  command {
    name            = "pythonshell"
    python_version  = "3.9"
    script_location = "s3://${aws_s3_bucket.driven_data_bucket_mykhailo.bucket}/tasks/dim_date.py"
  }
  tags = {
    Name = var.tag
  }
...
resource "aws_glue_job" "staging_dim_finance_glue" {
  ...
  command {
    name            = "pythonshell"
    python_version  = "3.9"
    script_location = "s3://${aws_s3_bucket.driven_data_bucket_mykhailo.bucket}/tasks/dim_finance.py"
  }
  tags = {
    Name = var.tag
  }
...
resource "aws_glue_job" "staging_dim_person_glue" {
  ...
  command {
    name            = "pythonshell"
    python_version  = "3.9"
    script_location = "s3://${aws_s3_bucket.driven_data_bucket_mykhailo.bucket}/tasks/dim_person.py"
  }
  tags = {
    Name = var.tag
  }
...
resource "aws_glue_job" "staging_fact_network_usage_glue" {
  ...
  command {
    name            = "pythonshell"
    python_version  = "3.9"
    script_location = "s3://${aws_s3_bucket.driven_data_bucket_mykhailo.bucket}/tasks/fact_network_usage.py"
  }
  tags = {
    Name = var.tag
  }
